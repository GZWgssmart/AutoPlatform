...
function initTable() {
    ...
    $("#cusTable").bootstrapTable({
        ...
        url: contextPath + "/accessoriesType/pager", //获取数据的Servlet地址
...
function showEditWin() {
    ...
    if (selectRow.length != 1) {
        ...
        $("#updateForm").fill(accessoriesType);
        $("#editWin").modal('show');
    }
}
...
function updateAccType() {
    $.post(contextPath + "/accessoriesType/update",
...
/**提交添加数据 */
function addAccessoriesType() {
    $.post(contextPath + "/accessoriesType/add",
        $("#addForm").serialize(),
        function(data){
            if(data.result == "success"){
                $('#addWin').modal('hide');
