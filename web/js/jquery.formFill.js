					...
					try {
						...
						try {
							...
							if (options.styleElementName == "object") {
								...
								if ($(item).attr("attr").match(/\[[0-9]*\]/i)) {
									objName = $(item).attr("attr").replace(/^[a-z]*[0-9]*[a-z]*\./i, 'obj.').replace(/\[[0-9]*\].*/i, "");

									arrayAtribute = $(item).attr("attr").match(/\[[0-9]*\]\.[a-z0-9]*/i) + "";
									arrayAtribute = arrayAtribute.replace(/\[[0-9]*\]\./i, "");
								} else {
									objName = $(item).attr("attr").replace(/^[a-z]*[0-9]*[a-z]*\./i, 'obj.');
								}
							} else if (options.styleElementName == "none") {
								objName = 'obj.' + $(item).attr("attr");
							}
